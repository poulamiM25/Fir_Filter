...
def filter_implementation(band,quantise_case):
    # construct sine wave
        ...
        # save dec converted filter output to csv
        def save_dec_filter_output(filename, filter_output):
            df = pd.DataFrame({"filter_output_python" : filter_output})
            df.to_csv(filename, index=False)
            
        filename_filter_output = "filter_output_python_" + quantise_case + "_" + band + ".csv"
        save_dec_filter_output(filename_filter_output,result_arr )
